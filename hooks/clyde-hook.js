...
//
// Fox Desktop Pet — Claude Code Hook Script
...
const EVENT_TO_STATE = {
  ...
  PreToolUse: "working",
  PostToolUse: "thinking",          // tool done → back to thinking (agent is deciding next step)
  PostToolUseFailure: "error",
  Stop: "attention",
  StopFailure: "error",
  SubagentStart: "juggling",
  SubagentStop: "idle",             // subagent done → back to idle (parent resumes on next event)
  PreCompact: "sweeping",
...
if (event === "SessionStart" && !process.env.CLYDE_REMOTE) getStablePid();
...
function send(sessionId, cwd, permissionMode) {
  ...
  if (!process.env.CLYDE_REMOTE) {
    ...
    if (_claudePid) {
      ...
      body.claude_pid = _claudePid; // backward compat with older Clyde versions
    }
