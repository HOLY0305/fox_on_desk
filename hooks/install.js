#!/usr/bin/env node
// Clyde Desktop Pet — Hook Installer
...
/**
 * Register Clyde hooks into ~/.claude/settings.json.
...
function registerHooks(options = {}) {
  ...
  let hookScript = path.resolve(__dirname, "clyde-hook.js").replace(/\\/g, "/");
  ...
  for (const event of hookEvents) {
    ...
    // Remote mode: prepend CLYDE_REMOTE=1 so the hook skips PID collection
    const desiredCommand = options.remote
      ? `CLYDE_REMOTE=1 node "${hookScript}" ${event}`
  ...
  if (options.autoStart) {
    ...
    if (!autoStartSync.found) {
      // Insert at index 0 — must run BEFORE clyde-hook.js so the app is starting
